...
import { getRPCEndpoint, getBalance } from './kryptonDB'
 ...
 * @param {string} url - URL of the KAZE node to query.
 ...
 * @param {string} net - 'MainNet', 'TestNet' or a custom KryptonDB url.
 * @param {string} scriptHash - Contract scriptHash
 ...
 * @param {number} streamCost - Amount of stream to pay for transfer.
 * @param {function} [signingFunction] - Optional external signing function.
 * @return {Promise<Response>} RPC response
 */
export const doTransferToken = (net, scriptHash, fromWif, toAddress, transferAmount, streamCost = 0, signingFunction = null) => {
  ...
  return Promise.all([rpcEndpointPromise, balancePromise])
    .then((values) => {
      ...
      const intents = [
        { assetId: ASSET_ID.STREAM, value: 0.00000001, scriptHash: fromAddrScriptHash }
      ]
      const invoke = { scriptHash, operation: 'transfer', args: [reverseHex(fromAddrScriptHash), toAddrScriptHash, transferAmount] }
      const unsignedTx = Transaction.createInvocationTx(balances, intents, invoke, streamCost, { version: 1 })
      if (signingFunction) {
