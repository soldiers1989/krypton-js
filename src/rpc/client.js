...
import { RPC_VERSION, DEFAULT_RPC, KAZE_NETWORK } from '../consts'
import logger from '../logging'
import { timeout } from '../settings'

const log = logger('rpc')

const versionRegex = /KAZE:(\d+\.\d+\.\d+)/
/**
 * @class RPCClient
 * @classdesc
 * RPC Client model to query a KAZE node. Contains built-in methods to query using RPC calls.
 * @param {string} net - 'MainNet' or 'TestNet' will query the default RPC address found in consts. You may provide a custom URL.
 * @param {string} version - Version of KAZE node. Used to check if RPC methods have been implemented. it will default to DEFAULT_RPC found in CONST
 */
class RPCClient {
  constructor (net, version = RPC_VERSION) {
    if (net === KAZE_NETWORK.MAIN) {
      this.net = DEFAULT_RPC.MAIN
    } else if (net === KAZE_NETWORK.TEST) {
  ...
  /**
   * Gets the version of the KAZE node. This method will never be blocked by version. This method will also update the current Client's version to the one received.
   * @return {Promise<string>}
   */
  getVersion () {
  ...
  /**
   * Checks if the provided address is a valid KAZE address.
   * @param {string} addr
   * @return {Promise<boolean>}
   */
