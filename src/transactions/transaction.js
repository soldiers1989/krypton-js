...
class Transaction {
   ...
   * @param {number} streamCost - Stream to attach for invoking script
   * @param {object} [override={}] - Optional overrides (eg.custom versions)
   * @param {number|Fixed8} [fees]
   * @return {Transaction} Unsigned Transaction
   */
  static createInvocationTx (balances, intents, invoke, streamCost = 0, override = {}, fees = 0) {
   ...
   * @param {string|Object} assetSymOrTxOut - The symbol of the asset (eg KAZE or STREAM) or the TransactionOutput object.
  ...
  /**
   * Calculate the inputs required based on existing outputs provided. Also takes into account the fees required through the stream property.
  ...
  calculate (balance, strategy = null, fees = 0) {
    const { inputs, change } = core.calculateInputs(balance, this.outputs, this.stream, strategy, fees)
    this.inputs = inputs
