...
/**
 * Verifies a KEP2. This merely verifies the format. It is unable to verify if it is has been tampered with.
 * @param {string} kep2
 * @return {boolean}
 */
export const isKEP2 = (kep2) => {
  try {
    if (kep2.length !== 58) return false
    const hexStr = ab2hexstring(base58.decode(kep2))
...
export const isAddress = (address) => {
  try {
    ...
    if (getAddressFromScriptHash(scriptHash) !== address) {
      // address is not valid Kaze address, could be btc, ltc etc.
      return false
    }
    return true
  } catch (e) { return false }
