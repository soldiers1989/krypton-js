...
/**
 * Calculate the inputs required given the intents and streamCost. streamCost has to be seperate because it will not be reflected as an TransactionOutput.
 * @param {Balance} balances - Balance of all assets available.
 * @param {TransactionOutput[]} intents - All sending intents
 * @param {number|Fixed8} extraCost - streamCost required for the transaction.
...
export const calculateInputs = (balances, intents, extraCost = 0, strategy = null, fees = 0) => {
  ...
  // Add STREAM cost and fees in
  extraCost = new Fixed8(extraCost).add(fees)
  if (extraCost.gt(0)) {
    if (requiredAssets[ASSET_ID.STREAM]) {
      requiredAssets[ASSET_ID.STREAM] = requiredAssets[ASSET_ID.STREAM].add(extraCost)
    } else {
      requiredAssets[ASSET_ID.STREAM] = extraCost
    }
  }
