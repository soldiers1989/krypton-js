...
- API
  ...
  - `setupVote` is a new method that allows you to setup your voting status for your kaze.
...
- Fixes
  ...
  - Added `sendingFromSmartContract` for claimStream.
...
- API
  ...
  - Kazescan `getTransactionHistory` is updated to use a paginated endpoint. It currently only supports a single page but it should speed up loading by a significant amount. The downside is that it will not return the full history.
...
- API
  ...
  - Add attribute and remark for signing empty transactions. Now if you use `doInvoke` for your token transfers, it will automatically setup your transaction to be accepted by the KAZE node without the need for any asset inputs.
  - `claimStream` now accepts a `claims` property as an override for claims. This behavior is inline with `sendAsset` and `doInvoke` accepting `balance` overrides.
...
- Fixes
  ...
  - Update `kazescan.getMaxClaimAmount` to use `get_unclaimed` endpoint
...
- Transaction
  ...
  ```js
  import { tx, settings } from '@kazeblockchain/krypton-js'
...
- Settings
  ...
  - There is now global settings that control how `krypton-js` works.  The first settings available are `httpsOnly` and `networks`.

    - `httpsOnly` is a boolean which will force kryptonDB and kazescan to return only https RPC nodes. Do note that an error will be thrown if no suitable nodes are found.
    - `networks` is an object containing every network configuration available for consumption. It comes default with `MainNet`, `TestNet` and `VertoNet`. For now, these configurations only determine the kryptonDB/kazescan urls used for each network but they will serve more purposes in the future.

- RPC

  - Network class representing a KAZE network. This can be created by importing a protocol file or just manually entering the parameters. This is the new way which we can link `krypton-js` to a private net that comes with a kazescan or kryptonDB setup.
...
- Fixes
  ...
  - Fix kryptonDB and kazescan `getTransactionHistory`. **Do note that this can be considered a breaking change for some as the return structure has changed. However, this is considered a fix as the original return structure was not intended.**
  - new Wallet to use default scrypt params when not provided.
  - Typescript typings refactored and cleaned up. Introduces the object-like interfaces which are the krypton-js classes exported as plain javascript objects.
...
- Fixes
  ...
  - kryptonDB to use `get_all_nodes` endpoint instead of `best_node` endpoint
  - Fix kazescan `getMaxClaimAmount`, `getBalance` and `getClaims` for invalid addresses
...
- API
  ...
  - Mark `api.kep5.doTransferToken` for deprecation.
...
- API
  ...
  - Expose the `loadBalance` method. This method allows the use of the dynamic API switch within `krypton-js` for the external providers.
  - Implement the full set of APIs on kazescan. Kazescan is now complete and up to par with kryptonDB in terms of features.
...
- Utils
  ...
  - Fixed8: A new class extending bignumber.js that replaces all value storage in krypton-js
...
- Wallet
  ...
  - `Claims` is now a ES6 class. While it does not have any spceial methods for it now, it is one of the high level objects that we will be working with in `krypton-js`. This allows easy creation of `Claims` using the constructor by passing in a CLaims-like javascript object.
  - `components` have been created for the minor sub-components found in `Claims` and `Balance`. These methods are useful for us to rapidly create components that are usable with `krypton-js` methods. Refer to the wallet section for more information.
  - **BREAKING** Excess KEP2 functions are now fully deprecated. The list is `encryptWifAccount`, `generateENcryptedWif`, `encryptWIF`, `decryptWIF`.

- Transaction

  - Update all methods to conform with the new Fixed8 classes. This also means that `krypton-js` will not be compatible with just normal javascript objects anymore.

- API

  - **BREAKING** Update external API libraries (kryptonDB and kazescan) to return `Balance` and `Claims` objects. Fixed8 will be used in the new models, making arithmetic operations very different from normal javascript numbers.
  - A bug has been discovered in `getPrice`. While it works for KAZE and STREAM, it will not work for any of the KEP5 tokens. `getPrices` has been fixed for this bug. However, there is no easy fix for `getPrice` and thus, we will move forward with deprecating `getPrice` in favor of `getPrices`.
