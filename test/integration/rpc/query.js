...
describe('Query', function () {
  ...
  describe('RPC Queries', function () {
    ...
    it('getAssetState', () => {
      return Query.getAssetState(ASSET_ID.KAZE)
        .execute(DEFAULT_RPC.TEST)
        .then((res) => {
          res.result.should.have.all.keys([
            'version',
    ...
    it('getPeers', () => {
      return Query.getPeers()
        .execute('http://node2.kaze.solutions:44886')
        .then((res) => {
          res.result.should.have.all.keys([
            'unconnected',
            'connected',
    ...
    it('getVersion', () => {
      return Query.getVersion()
        ...
        .then((res) => {
          ...
          res.result.useragent.should.match(/KAZE:(\d+\.\d+\.\d+)/)
        })
    })
    ...
    describe('invoke', () => {
      it('simple', () => {
        return Query.invoke(CONTRACTS.TEST_RPX, ContractParam.string('name'), ContractParam.boolean(false))
          .execute(DEFAULT_RPC.TEST)
          .then((res) => {
            res.result.should.have.all.keys(['script', 'state', 'stream_consumed', 'stack'])
            res.result.state.should.equal('HALT, BREAK')
            res.result.stack[0].value.should.equal('5265642050756c736520546f6b656e44886e312e34')
          })
      })
