 ...
 * The core functionality are sendAsset, claimStream and doInvoke.
 ...
 * @param {string} config.net - 'MainNet', 'TestNet' or a krypton-wallet-db URL.
 ...
 * @return {Promise<object>} Configuration object.
 */
export const claimStream = config => {
  return fillUrl(config)
    .then(fillKeys)
    .then(fillClaims)
    ...
    .catch(err => {
      ...
      log.error(`claimStream failed with ${err.message}. Dumping config`, dump)
      throw err
    })
}
 ...
 * @param {number} config.stream - streamCost of VM script.
 * @param {bool} [config.sendingFromSmartContract] - Optionally specify that the source address is a smart contract that doesn't correspond to the private key.
 * @return {Promise<object>} Configuration object.
 */
...
export const createTx = (config, txType) => {
  ...
  switch (txType) {
    ...
    case 'invocation':
    case 209:
      checkProperty(config, 'balance', 'stream', 'script')
      if (!config.intents) config.intents = []
      tx = Transaction.createInvocationTx(config.balance, config.intents, config.script, config.stream, config.override, config.fees)
      break
    case 'state':
    case 144:
 ...
 * @param {string} config.url - KAZE Node URL.
  ...
  return Query.sendRawTransaction(config.tx)
    ...
    .then(res => {
      ...
      if (res.result === true) {
        ...
        const dump = {
          net: config.net, address: config.address, intents: config.intents, balance: config.balance, claims: config.claims, script: config.script, stream: config.stream, tx: config.tx
        }
        log.error(
 ...
 * @param {number} assetAmts.KAZE - Amt of KAZE to send.
 * @param {number} assetAmts.STREAM - Amt of STREAM to send.
 ...
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint API object. eg, kryptonDB or Kazescan.
...
/**
 * Helper method to get the maximum amount of stream claimable after spending all KAZE.
 * @param {object} config - Configuration object.
 * @param {string} config.net - 'MainNet', 'TestNet'
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint APi object. eg, kryptonDB or Kazescan.
 * @return {Promise<Fixed8>} max claimable STREAM
 */
